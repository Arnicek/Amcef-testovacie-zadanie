function canceledTransactions(transactions) {
  let result = {};

  let canceled = transactions.filter(t => t.state == "canceled");

  canceled.sort((t1, t2) => t2.createdAt - t1.createdAt);

  for (let i = 0; i < canceled.length; i++) { 
    let year = canceled[i].year;
    if (result[year] === undefined) { 
      result[year] = []; }
    result[year].push(canceled[i]);
  }
...
console.log(canceledTransactions(transactions))
